...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const closeWalletHistoryModal = document.getElementById('closeWalletHistoryModal');
    ...
    // Open wallet modal when clicking ADD FUNDS button
    if (addFundsBtn) {
    ...
    // Open wallet history modal when clicking VIEW HISTORY button
    if (viewHistoryBtn) {
    ...
    // Close wallet modal when clicking close button
    if (closeWalletModal) {
    ...
    // Close wallet history modal when clicking close button
    if (closeWalletHistoryModal) {
        closeWalletHistoryModal.addEventListener('click', function() {
            closeWalletHistoryModalFunc();
        });
    }

    // Close wallet modal when clicking cancel button
    if (cancelWalletBtn) {
    ...
    // Close wallet history modal when clicking outside
    if (walletHistoryModal) {
        walletHistoryModal.addEventListener('click', function(e) {
            if (e.target === walletHistoryModal) {
                closeWalletHistoryModalFunc();
            }
        });
    }
    ...
    quickSelectBtns.forEach(btn => {
        btn.addEventListener('click', function(e) {
            ...
            walletAmountInput.value = amount;
    ...
    if (walletAmountInput) {
        walletAmountInput.addEventListener('input', function() {
            // Remove active state from quick select buttons when typing
            quickSelectBtns.forEach(b => b.classList.remove('active'));
        });
    }
    ...
    if (walletForm) {
        walletForm.addEventListener('submit', function(e) {
            ...
            const paymentMethod = document.getElementById('walletPaymentMethod').value;

            if (!amount || amount < 100) {
                alert('Please enter an amount of at least Rs.100');
                return;
            }
            ...
            const formattedAmount = 'Rs.' + parseInt(amount).toLocaleString();
            ...
            // Show success message
            alert('Processing payment of ' + formattedAmount + '...\nFunds will be added to your wallet shortly!');
    ...
    function openWalletHistoryModal() {
        if (walletHistoryModal) {
            walletHistoryModal.classList.add('active');
            document.body.style.overflow = 'hidden';
        }
    }

    function closeWalletHistoryModalFunc() {
        if (walletHistoryModal) {
    ...
    // Close wallet modal with Escape key
    ...
    // Close wallet history modal with Escape key
    document.addEventListener('keydown', function(e) {
        if (e.key === 'Escape' && walletHistoryModal && walletHistoryModal.classList.contains('active')) {
            closeWalletHistoryModalFunc();
        }
    });
